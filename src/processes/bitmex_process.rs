...
use bincode::config::{WithOtherTrailing, WithOtherIntEncoding, FixintEncoding, AllowTrailing};
...
type CustomBincode = WithOtherTrailing<WithOtherIntEncoding<DefaultOptions, FixintEncoding>, AllowTrailing>;
...
pub struct BitmexProcess<'a> {
    ...
    quotes_guard_: tracing_appender::non_blocking::WorkerGuard,
    trades_guard_: tracing_appender::non_blocking::WorkerGuard,
...
impl<'a> BitmexProcess<'a> {
    pub fn new(coin: &'a str) -> Self {
        ...
        let quotes_appender = tracing_appender::rolling::daily(
            settings.data_quotes_path,
            file_name_prefix.clone(),
        );
        let (non_blocking_quotes_appender, quotes_guard_) =
            tracing_appender::non_blocking(quotes_appender);

        let trades_appender = tracing_appender::rolling::daily(
            settings.data_trades_path,
            file_name_prefix,
        );
        let (non_blocking_trades_appender, trades_guard_) =
                    ...
                    if message.find("table").is_some() =>
                {
                    ...
                    match bitmex_msg {
                        BitmexParser::Quotes(quotes) => {
                            ...
                            self.quotes_appender.write_all(&bin_quotes[Self::USIZE_LEN..]).unwrap();
                            self.quotes_appender.flush().unwrap();
                        },
                        BitmexParser::Trades(trades) => {
                            ...
                            self.trades_appender.write_all(&bin_trades[Self::USIZE_LEN..]).unwrap();
                            self.trades_appender.flush().unwrap();
                        },
                        _ => (),
                    }
                }
